...
- **`deploy.yml`** (BE): chạy khi push vào `main` (hoặc bấm tay qua *Run workflow*), 3 job tuần tự:
...
- **Deploy lại không migrate**: tab *Actions* → *Deploy Railway* → *Run workflow* → tick `skip_migrate`.
- **Chỉ redeploy (không build Docker)**: *Run workflow* → tick `skip_docker_push`.
